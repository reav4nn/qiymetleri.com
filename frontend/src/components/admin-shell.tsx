...
export function AdminShell({
  ...
  const sidebar = (
    <>
        ...
        {nav.map(([href, label, Icon]) => {
          ...
          return (
            <Link
              ...
              href={target}
              onClick={() => setOpen(false)}
              className={`flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-semibold ${active ? "bg-red-600 text-white" : "text-zinc-400 hover:bg-white/10 hover:text-white"}`}
            >
              <Icon size={19} />
              {label}
            </Link>
      ...
      <button
        onClick={logout}
        className="m-3 flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-semibold text-zinc-400 hover:bg-white/10 hover:text-white"
      >
        <LogOut size={19} />
        Çıxış
      </button>
