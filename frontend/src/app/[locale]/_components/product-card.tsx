...
import type { Product } from "@/app/[locale]/_lib/mock-data";
  ...
  return (
    ...
    >
      {product.discountPercent ? (
        <span className="absolute top-[24px] left-[24px] z-2 rounded-badge bg-accent px-2 py-1 text-xs font-extrabold text-white">
          −{product.discountPercent}%
        </span>
      ) : null}
      ...
      <div
        className="flex h-[150px] items-center justify-center rounded-button"
        style={{
          backgroundImage:
            "repeating-linear-gradient(45deg, #f4f4f5 0, #f4f4f5 10px, #fafafa 10px, #fafafa 20px)",
        }}
      >
        <span className="font-mono text-[11px] text-[#a1a1aa]">{product.imageHint}</span>
      </div>
      ...
      <div>
        ...
        <div className="flex items-baseline justify-between">
          <span className="text-xl font-extrabold tracking-[-0.02em]">
            {format.number(product.price)} {t("product.unit")}
          </span>
